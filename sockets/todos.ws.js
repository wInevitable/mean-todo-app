...
module.exports = function(io) {
  ...
  todos.on('connection', function(socket) {
    ...
    socket.on('saveTodo', function(todo) {
      todosDB.saveTodo(todo, function(error, data) {
        if (error) throw error; // You can emit the error to a socket
    ...
    socket.on('updateTodo', function(data) {
      todosDB.updateTodo(data, function(error, data) {
        if (error) throw error; // You can emit the error to a socket
    ...
    socket.on('deleteTodo', function(data) {
      todosDB.deleteTodo(data, function(error, data) {
        if (error) throw error; // You can emit the error to a socket
  ...
  function dispatchAll(socket) {
    todosDB.getAllTodos(function(error, data) {
      if (error) throw error; // You can emit the error to a socket
